...
`v1.6.10-alpha` adds additional candidate columns to the score output:
...
These scores are fixed formulas over role-stripped features. They are not learned from targets.

## Lane-to-target map

| lane | candidate score | evaluation target |
|---|---|---|
| density pressure | `shadow_density_pressure_score` | `target_false_pressure_density_rate` |
| raw false-one | `shadow_raw_false_one_pressure_score` | `target_raw_false_one_rate` |
| demotion | `shadow_demotion_pressure_score` | `target_false_one_demotion_rate` |
| hold / demote | `shadow_hold_or_demote_pressure_score` | `target_hold_or_demote_rate` |
| relation-specific | `shadow_relation_specific_pressure_score` | `target_relation_false_pressure_share` |
| return-specific | `shadow_return_specific_pressure_score` | `target_return_false_pressure_share` |
| native breach proxy | `shadow_native_breach_proxy_score` | `target_native_breach_rate` |
...
After a hardened evidence base exists and the v1.6.10 score report has been regenerated:
...
& $P -m zerogate_sim.shadow_lane_discrimination_report `
  --evidence-base $Base `
  --out runs\shadow_lane_discrimination_v1_6_10_triad27
```

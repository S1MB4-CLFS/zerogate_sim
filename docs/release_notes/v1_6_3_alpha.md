...
`v1.6.4-alpha` now performs four-gate reconciliation / return-adversary audit before holdout. `v1.6.5-alpha` adds the held-out `deep81` / `wide243` role-stripped evaluation and preserves exact-baseline honesty if the feature schema does not provide every baseline.

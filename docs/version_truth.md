...
**Status:** README overflow moved here in `v1.6.9-alpha`; current spine advanced in `v1.6.24-alpha`  
...
The active public line is now `v1.6.24-alpha` — How It Works and v1.6 closeout route lock.
...
- `v1.6.19-alpha` — Four Gates debt candidate generator: `four_gates_debt` near-success candidate profile for the next triad27 debt evidence gate.
...
- `v1.6.0-alpha` adds role-blind shadow design, a report schema, and falsifier boundaries before scoring code exists.
...
- `v1.6.9-alpha` adds shadow discrimination repair: residual target diagnostics after the best available dumb baseline and README boundary-list relocation.
- `v1.6.10-alpha` adds lane-specific candidate scores and no-delete runs inventory classification.
...
- `v1.6.19-alpha` adds the `four_gates_debt` candidate generator profile and generator report before heavy evidence.
...
`v1.6.19-alpha` is the Four Gates debt candidate generator gate. It preserves `C_Z = min(D, P, R, B)`, does not start Zenodo correction, does not revive shadow, and does not claim observed-universe proof. It implements the `four_gates_debt` near-success candidate profile before the `v1.6.20-alpha` triad27 debt evidence gate.


## v1.6.20-alpha note

`v1.6.20-alpha` is the Four Gates triad27 debt evidence gate. It preserves `C_Z = min(D, P, R, B)`, does not start Zenodo correction, does not revive shadow, and does not claim observed-universe proof. It tests whether relation debt and return debt become visible structured-zero lanes under `triad27` debt-shaped candidates.


## v1.6.21-alpha note

`v1.6.21-alpha` is the Four Gates deep81 / wide243 debt evidence gate. It preserves `C_Z = min(D, P, R, B)`, does not start Zenodo correction, does not revive shadow, and does not claim observed-universe proof. It tests whether the `v1.6.20-alpha` triad27 relation-debt and return-debt pattern survives perturbation and temporal-depth weather.
...
`v1.6.24-alpha` is the How It Works and closeout route lock gate. It preserves `C_Z = min(D, P, R, B)`, does not start Zenodo correction, does not revive shadow, and does not claim observed-universe proof. It adds the current evidence index and anti-tautology audit plan before the final v1.6 audit and package gates.

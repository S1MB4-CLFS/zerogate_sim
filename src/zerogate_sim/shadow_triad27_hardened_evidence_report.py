...
def _write_role_stripped_files(output_dir: Path, cell_rows: list[dict[str, object]]) -> dict[str, Path]:
    ...
    profile_features = [_profile_feature_row(cell_rows)]
    family_features = _family_feature_rows(cell_rows)
    ...
    lines = [
        ...
        "",
        "Feature rows may include observable weather axes, pressure rates, and exact baseline fields. They must not include native gate labels, truth roles, candidate profiles, answer keys, or target fields.",
        "",
...
def write_shadow_triad27_hardened_evidence_report(*, output_dir: Path, matrix_dirs: Iterable[Path]) -> dict[str, Path]:
    ...
    _write_csv(cell_features_path, _family_feature_rows(cell_rows))

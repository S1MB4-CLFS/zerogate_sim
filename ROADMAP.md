...
The controlled synthetic-field language boundary remains active. This roadmap is not an observed-universe or physics-claim route.

ZeroGateSim tests whether dimensional expression can be modeled as candidate freedoms passing through four gates:

```text
Distinction -> Polarity -> Relation -> Return
```
...
| evidence gate | decision | important caveat |
|---|---|---|
| `v1.6.20` triad27 debt | expand | small-weather only |
...
No new theory branch belongs in v1.6. The remaining versions are verification and packaging gates.
...
The stronger public wording must stay bounded:
...
## Pre-debt native stepping stones

These remain because they explain the current evidence route without reopening the full historical wall:
...
| `v1.6.18-alpha` | Four Gates debt candidate design and logic cleanup |
| `v1.6.19-alpha` | debt candidate generator |
| `v1.6.20-alpha` | triad27 debt evidence |
...
```text
docs/v1_7_witness_spectrum.md
docs/v1_7_perturbation_curve.md
docs/v1_7_weather_curve_summary.md
docs/v1_7_expected_quiet_lane_activation.md
tests/test_v1_7_perturbation_spectrum.py
```
...
`v1.7.7-alpha` audits the completed fresh holdout ladder before reviewer packaging. This repeats the useful v1.6.x lesson: do not package a result until the result has faced anti-tautology and role-dependence pressure.

The audit follows a known empirical routine:
...
```text
The completed triad27/deep81/wide243 holdout ladder is non-vacuous, non-dead-safe, candidate-name masked, role-bounded, and not merely true by definition.
```
...
post-hoc expected manifest,
missing weather rung,
candidate-name / role-label leakage,
dead-safe no-crown behavior,
false-one pressure absent,
...
The old release spine, shadow route, legacy evidence visuals, and README/ROADMAP snapshots are preserved in:

- [`docs/history_vault/README.md`](docs/history_vault/README.md)
- [`docs/version_truth.md`](docs/version_truth.md)

No deletion by vibes. No historical erasure. The vault keeps what was; the README shows what is; this roadmap shows what can be.
